...
function usePaymentStatus(email: string | null | undefined): PaymentStatus {
  const [status, setStatus] = useState<PaymentStatus>(null);
  useEffect(() => {
    if (!email) return;
    fetch(`/api/payment-notification-status?email=${encodeURIComponent(email)}`)
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-[#07111f] text-white">
      ...
      <main className="mx-auto max-w-lg px-3 py-6 space-y-3 sm:px-4 sm:py-8 sm:space-y-4">
        ...
        {!subscription && paymentStatus === 'pending' && (
          <div className="rounded-2xl border border-blue-500/30 bg-[#06101e] overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4">
              {/* Dönen ikon */}
              <div className="shrink-0 flex h-10 w-10 items-center justify-center rounded-full bg-blue-500/15 border border-blue-500/30">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-white">Ödemeniz Kontrol Ediliyor</p>
                <p className="mt-0.5 text-xs text-[#5a6a80]">Bildiriminiz alındı, en kısa sürede hesabınız aktif edilecek.</p>
              </div>
            </div>
            <div className="border-t border-blue-500/15 px-5 py-3 flex items-center justify-between">
              <p className="text-[11px] text-[#4b5563]">Genellikle 5–15 dakika içinde onaylanır.</p>
        ...
        {/* Reddedilmiş ödeme uyarısı */}
        {!subscription && paymentStatus === 'rejected' && (
          <div className="rounded-2xl border border-red-500/30 bg-red-950/20 px-5 py-4 flex items-start gap-3">
            <span className="text-xl shrink-0">❌</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-red-400">Ödeme Onaylanamadı</p>
              <p className="mt-0.5 text-xs text-[#5a6a80]">Ödemeniz doğrulanamadı. Lütfen WhatsApp üzerinden destek alın.</p>
              <a href="https://wa.me/447441921660" target="_blank" rel="noopener noreferrer"
