...
// Tam MDX pipeline kuruluysa bu fonksiyonu kaldırıp doğrudan MDX kullanın.
function renderMarkdown(md: string): string {
  return md
    ...
    .replace(/^# (.+)$/gm,   '<h1 class="mt-0 mb-4 text-2xl font-bold text-white">$1</h1>')
    // Bold
    .replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-white">$1</strong>')
    // Tablo satırları
    .replace(/^\|(.+)\|$/gm, (row) => {
      if (/^[\|\s\-:]+$/.test(row)) return ''; // ayraç satırı
      const cells = row.split('|').filter(Boolean).map((c) => c.trim());
      const isHeader = cells.some((c) => /^[-\s]+$/.test(c));
      if (isHeader) return '';
      return `<tr class="border-b border-white/[0.08]">${cells.map((c) =>
        `<td class="px-3 py-2 text-sm text-[#9b98b0]">${c}</td>`
      ).join('')}</tr>`;
    })
    // Liste maddeleri
    .replace(/^[-*] (.+)$/gm, '<li class="flex items-start gap-2 text-sm text-[#9b98b0]"><span class="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-[#7c6fcd]"></span>$1</li>')
    // Numaralı liste
    .replace(/^\d+\. (.+)$/gm, '<li class="text-sm text-[#9b98b0] list-decimal ml-5">$1</li>')
    // Paragraf (boş satırlar)
    .replace(/\n\n/g, '</p><p class="leading-relaxed text-[#9b98b0]">')
    // Link
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" class="text-[#7c6fcd] underline underline-offset-2 hover:text-white transition-colors">$1</a>'
    )
    .trim();
}
