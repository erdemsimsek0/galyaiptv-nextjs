...
async function recordTrial(email: string, ip: string, selectedPackage: string) {
  const record: TrialRecord = { email, ip, createdAt: Date.now(), selectedPackage };
    ...
    html: `
      <div style="font-family:Arial,sans-serif;background:#0b0b0f;padding:24px;">
        <div style="max-width:560px;margin:0 auto;background:#111827;border:1px solid #7c3aed;border-radius:16px;padding:32px">
          ...
          <p style="color:#d1d5db;font-size:15px;line-height:1.6">Ücretsiz test talebinizi doğrulamak için aşağıdaki 6 haneli kodu kullanın:</p>
...
async function sendTrialMail(email: string, username: string, password: string) {
  ...
  await createTransporter().sendMail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background-color:#ffffff;border:1px solid #e0e6ed;">
        ...
        <div style="background:linear-gradient(135deg,#512da8,#7c3aed);color:#ffffff;padding:30px;text-align:center;border-radius:8px;">
          ...
          <a href="${whatsappUrl}" style="background-color:#25d366;color:#ffffff;padding:12px 28px;text-decoration:none;border-radius:25px;font-weight:bold;font-size:15px;display:inline-block;margin-top:10px;">💬 WhatsApp ile Satın Al</a>
        </div>
        <div style="margin-top:20px;background-color:#ffffff;padding:20px;border-radius:8px;border:1px solid #e0e6ed;">
          ...
          <p style="color:#555;margin:0 0 15px;font-size:14px;">En yaygın yöntemdir. Uygulamanızda bu alanları doldurun.</p>
          <table style="width:100%;font-size:15px;border-collapse:collapse;">
            <tr><td style="width:35%;color:#666;padding:8px 0;"><strong>Sunucu:</strong></td><td style="font-family:monospace;color:#333;font-size:13px;">http://pro4kiptv.xyz:2086/</td></tr>
            <tr style="background-color:#f9f5ff;"><td style="color:#666;padding:8px;"><strong>Kullanıcı Adı:</strong></td><td style="font-family:monospace;color:#512da8;font-size:14px;font-weight:bold;padding:8px;background:#f0ebff;">${username}</td></tr>
            <tr><td style="color:#666;padding:8px 0;"><strong>Şifre:</strong></td><td style="font-family:monospace;color:#512da8;font-size:14px;font-weight:bold;padding:8px;background:#f0ebff;border-radius:4px;">${password}</td></tr>
          </table>
        </div>
        ...
        <div style="margin-top:20px;background:linear-gradient(135deg,#512da8,#7c3aed);padding:24px;border-radius:8px;text-align:center;">
          ...
          <a href="${whatsappUrl}" style="background-color:#25d366;color:#ffffff;padding:14px 32px;text-decoration:none;border-radius:25px;font-weight:bold;font-size:16px;display:inline-block;">💬 WhatsApp: +44 7441 921660</a>
        </div>
        <div style="text-align:center;margin-top:30px;border-top:1px solid #e0e6ed;padding-top:20px;">
          ...
          <p style="font-size:12px;color:#999;margin:6px 0 0;">© 2026 Galya Media. Tüm hakları saklıdır.</p>
          <p style="font-size:12px;color:#999;margin:4px 0 0;">Bu test hesabı 12 saat sonra otomatik devre dışı kalacaktır.</p>
        </div>
      </div>`,
  });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'send_otp') {
      ...
      if (existing) {
        ...
        return NextResponse.json(
          { success: false, alreadyUsed: true, error: `Bu e-posta veya IP ile daha önce test oluşturulmuş. ${daysLeft} gün sonra tekrar deneyebilirsiniz.` },
          { status: 429 }
        );
      }
      const generatedOtp = generateOtp();
      const generatedToken = generateToken();
      await saveOtp(generatedToken, { email, otp: generatedOtp, ip, selectedPackage: selectedPackage || 'Belirtilmedi' });
    ...
    if (action === 'verify') {
      ...
      const existing = await findExistingTrial(email, ip);
      if (existing)
        return NextResponse.json({ success: false, alreadyUsed: true, error: 'Bu e-posta veya IP ile daha önce test oluşturulmuş.' }, { status: 429 });

      const creds = await createTrialUser();
      await recordTrial(email, ip, record.selectedPackage);
