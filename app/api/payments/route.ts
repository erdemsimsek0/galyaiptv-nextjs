...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      if (isAdmin && body.action === 'assign_subscription') {
        ...
        if (username && password) {
          ...
          const trialData = existingTrial
            ? JSON.parse(existingTrial)
            : { email, ip: 'admin-assigned', createdAt: Date.now() };
        ...
        return NextResponse.json({
          success: true,
          message: `${email} için ${plan} - ${durationDays} günlük abonelik tanımlandı.`,
        });
      }
    ...
    // ── FormData body (kullanıcı ödeme bildirimi) ────────────────────────────
    ...
    return NextResponse.json({
      success: true,
      message: 'Ödeme bildirimi alındı. En kısa sürede aktif edilecektir.',
    });
